...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    ...
    val currentOnDestinationOnChanged by rememberUpdatedState(newValue = onToDestinationChanged)
    LaunchedEffect(key1 = onToDestinationChanged) {
        snapshotFlow { editableUserInputState.text }
            .filter { !editableUserInputState.isHint }
            .collect {
                currentOnDestinationOnChanged(editableUserInputState.text)
            }
    }
}
